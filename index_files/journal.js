...
Journal.enableStickyHeader = function (padding) {
    ...
    $(window).scroll(function() {
        ...
        if ($(window).scrollTop() > 1 * (Journal.stickyHeaderHeight) && $(window).width() >= 768) {
...
Journal.productPageGallery = function () {
    if (!$('html').hasClass('quickview')) {
        ...
        $('#image').parent().click(function () {
            $('.product-info .image-gallery a.swipebox').eq($(this).find('img').attr('data-src-index') || 0).click();
            return false;
        });
...
Journal.updateProductPrice = function () {
    $.ajax({
        ...
        type: 'post',
        data: $('.product-info input[type=\'text\'], .product-info input[type=\'hidden\'], .product-info .this-visible input[type=\'radio\']:checked, .product-info input[type=\'checkbox\']:checked, .product-info select, .product-info textarea'),
        dataType: 'json',
        success: function (json) {
			
            //$('.product-info .price .price-old, .product-info .price .product-price .autocalc-product-price').text(json.price);
			$('.product-info .price .price-old, .product-info .price .product-price .autocalc-product-price').attr('data-value',parseFloat(json.price));
...
Journal.enableProductOptions = function () {
    ...
    $(document).on('change','.product-info .options:not(.push-select) .option select',function () {
        Journal.updateProductPrice();
    });

    /* change image for checkbox and radio options */
    $(document).on('click','.product-info .options:not(.push-radio) .option input[type="radio"]',function () {
    
        $(document).ajaxSuccess(function (event, xhr, settings) {
            let ajax_url = settings.url;
            if ((ajax_url.indexOf('index.php?route=product/product/dependentoption') !== -1) == true) {
                Journal.updateProductPrice();
            }
        });
...
Journal.enableQuickView = function () {
    ...
    $('.product-wrapper .image:not(.not), .product-list-item .image:not(.not)').each(function () {
...
Journal.searchAutoComplete = function () {
    ...
    $('div.journal-search input').autocomplete2({
...
//                    name = suggestion.value.replace(new RegExp(pattern, 'gi'), '<strong>$1<\/strong>');
    ...
    $('div.mobilesearch input').autocomplete2({
        appendTo: 'div.mobilesearch > div',
        width: '100%',
        serviceUrl: 'index.php?route=journal2/search',
        deferRequestBy: 500,
        paramName: 'search',
...
Journal.onMobileOrTablet = function () {
    ...
    /* ajax cart */
    $('div#cart .heading').die('mouseleave').die('mouseover').die('mouseleave').die('click');
    $('div#cart .heading').live('click', function () {
		
        if (!$("div#cart").hasClass('active')) {
            if (!Journal.isOC2) {
...
Journal.onDesktop = function () {
    ...
    /* ajax cart event*/
    $('div#cart .heading').die('click');
    $('div#cart').die('mouseleave').die('mouseover').die('mouseleave').die('click');
...
if (Journal.isOC2) {
    cart.add = function(product_id, quantity) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            beforeSend: function() {
                $('div#cart > button > a > span').button('loading');
            },
            complete: function() {
                $('div#cart > button > a > span').button('reset');
            },
            success: function(json) {
                if (json['redirect']) {
                ...
                if (json['success']) {
                    ...
                    setTimeout(function () {
                        $('header').find('span#cart-total').html(json['total']);
                    }, 100);
                    ...
                    $('div#cart ul').load('index.php?route=common/cart/info ul li');
                }
            }
        });
    };
        ...
        $.ajax({
            ...
            success: function(json) {
                ...
                if (getURLVar('route') == 'checkout/cart' || getURLVar('route') == 'checkout/checkout') {
                    ...
                    $('#divcart ul').load('index.php?route=common/cart/info ul li');
                }
            },
...
function addToCart(product_id, quantity) {
    $('.top-cart div#cart').css({'width': '100%'});
